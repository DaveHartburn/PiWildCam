<?php
/* PHP script to show status and allow config
 * By Dave Hartburn 2015
 */


$apssid="wildcam";
$binpath="/home/pi/wildbin";
$roothelp="/home/pi/wildbin/roothelp";
$logfile="/home/pi/wildcaps/wildcam.log";
...
if($_REQUEST['press']=="1") {
	...
	preg_match('/ESSID:"(.*)"/', $cmdout, $matches);
	//print_r($matches);
	$ssid=$matches[1];
	if($ssid == $apssid) {
	...
	$output=`ps -ef | grep 'wildcam.py' |grep -v 'grep' > /dev/null`;
	if($output=="") {
		print "Not running";
		start_cam_opts();
		$cambut="Run Wildcam";
	...
	//exec("ps -eo args | grep '^/usr/local/bin/mjpg_streamer' > /dev/null", $output, $result);
	$output=`pidof mjpg_streamer`;
	if($output=="") {
		print "Not running";
		$streambut="Streamer On";
	} else {
		$link="http://".$_SERVER['HTTP_HOST'].":8080/?action=stream";
		#print "Running (<a href=\"$link\">$link</a>)";
		print("<iframe src=\"$link\" width=640 height=480></iframe>");
		$fulllink="http://".$_SERVER['HTTP_HOST'].":8080";
		print("<br><a href=\"$fulllink\">$fulllink</a>");
	...
	print " or paste \\\\".$_SERVER['HTTP_HOST']."\\wildcaps\\ into explorer</td>\n";
...
sub process_input {
	...
	if($var eq "Run Wildcam") {
		...
		print FH "Command run, output is: $cmdout\n";
		close(FH);
	}
...
function myPhpInfo() {
	// Lighttpd does not print php info as nicely as apache. Wrap in pre
	print("<pre>");
	phpInfo();
	print("</pre>");
}
